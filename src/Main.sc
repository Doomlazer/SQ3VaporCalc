...
(instance logFile of gamefile_sh
	(properties)
)
...
code_0536:
	...
	(method (handleEvent pEvent &tmp temp0 temp1 newEvent temp3 temp4 pEventX pEventY pEventModifiers temp8 [temp9 50])
		(if (pEvent claimed?) (return))
		(super handleEvent: pEvent)
		(if (== global252 1)
			((ScriptID 30) handleEvent: pEvent)
			(if (== global252 0)
				(pEvent claimed: 1)
				((ScriptID 30) dispose:)
			)
					...
					((Said 'get')
						(cond 
							(
								(or
									(not global200)
									(not (= temp0 (gInv firstTrue: #saidMe)))
								)
								(proc255_0 0 20)
							)
				...
				(if global200
					...
					(cond 
						...
						(== (= pEventModifiers (pEvent modifiers?)) 10)
							(pEvent claimed: 1)
							((User alterEgo?) setMotion: JumpTo pEventX pEventY)
						)
						((& pEventModifiers emSHIFT)
							(pEvent claimed: 1)
							(= temp1
								(proc255_0
									(Format @temp9 0 29 pEventX pEventY)
									67
									(cond 
										((< pEventX 20) pEventX)
										((< 300 pEventX) (- pEventX 40))
										(else (- pEventX 20))
									)
									(if (< pEventY 16) pEventY else (- pEventY 6))
									33
									999
									91
								)
						...
						((& pEventModifiers emALT) (pEvent claimed: 1) ((User alterEgo?) showSelf:))
					)
				)
			)
			(evKEYBOARD
				...
				(switch (pEvent message?)
					...
					(KEY_ALT_z
						(if global200 (pEvent claimed: 1) (= global4 1))
					)
					(KEY_ALT_e
						(proc255_0
							(Format
